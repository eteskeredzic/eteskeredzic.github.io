int[][] b = new int[4][4];
int pad = 20, bs = 100, len = pad * (b.length + 1) + bs * b.length * 4;
int score = 0, dead = 0;
...
void draw() {
 ...
 textt("Score: " + score, 10, 5, 100, 50, color(0), 15, LEFT);
 if (dead != 0) {
  rectt(0, 0, width, height, 0, color(255, 100));
  textt("Game over! Click to restart", 0, height / 2, width, 50, color(0), 30, CENTER);
  ...
  for (int perp = 0; perp < b.length; perp++)
   for (int tang = (d > 0 ? b.length - 2 : 1); tang != (d > 0 ? -1 : b.length); tang -= d) {
    ...
    for (int i = (dx != 0 ? x : y) + d; i != (d > 0 ? b.length : -1); i += d) {
     ...
     if (bak[r][c] != 0 && bak[r][c] != b[y][x]) break;
     if (dx != 0) tx = i;
     else ty = i;
    }
    if ((dx != 0 && tx == x) || (dy != 0 && ty == y)) continue;
    else if (bak[ty][tx] == b[y][x]) {
     bak[ty][tx] *= 2;
...
boolean gameover() {
  ...
  boolean out = true;
  for (int i = 0; i < 4; i++)
   if (go(dy[i], dx[i], false) != null) out = false;
  return out;
}
